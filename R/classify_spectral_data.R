...
classify_spectral_data <-function(LDIRData, SpecLib, HitRate = 0.8, SizeClasses = c(0,10,50,100,200,500,900)) {
  ...
  # remove mu symbol to prevent packaging errors
  base::names(df.loopdata)[base::names(df.loopdata) == 'Diameter..\u00B5m.'] <- 'Diameter..microns.'
  ...
  df.loopdata$diameter.binned <- base::cut(df.loopdata$Diameter..microns.,SizeClasses)
